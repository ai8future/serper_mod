...
```go
import chassiserrors "github.com/ai8future/chassis-go/v10/errors"
...
- **Library has no CLI dependencies.** The `serper` package only imports `chassis-go/v10/errors` and `chassis-go/v10/secval` (both pure functions, no I/O). Config loading, logging, and retry logic are CLI-only concerns.
...
- **Version gate** (`chassis.RequireMajor(9)`) at CLI startup prevents silent ABI mismatches if the chassis-go major version changes.
...
Built on [chassis-go/v10](https://github.com/ai8future/chassis-go), an internal Go framework:

| Package | Used By | Purpose |
|---------|---------|---------|
| `chassis` | CLI | Version gate (`RequireMajor(9)`) |
| `chassis-go/v10/errors` | Library | Typed error constructors with HTTP + gRPC codes |
| `chassis-go/v10/secval` | Library | JSON response security validation |
| `chassis-go/v10/call` | CLI | Resilient HTTP client (retry + timeout + OTel tracing) |
| `chassis-go/v10/config` | CLI | Struct-tag-based environment variable config loader |
| `chassis-go/v10/logz` | CLI | Structured JSON logger |
| `chassis-go/v10/testkit` | CLI tests | Test environment helpers |
