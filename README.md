...
```go
import chassiserrors "github.com/ai8future/chassis-go/v11/errors"
...
- **Library has no CLI dependencies.** The `serper` package only imports `chassis-go/v11/errors` and `chassis-go/v11/secval` (both pure functions, no I/O). Config loading, logging, and retry logic are CLI-only concerns.
...
- **Version gate** (`chassis.RequireMajor(11)`) at CLI startup prevents silent ABI mismatches if the chassis-go major version changes.
...
Built on [chassis-go/v11](https://github.com/ai8future/chassis-go), an internal Go framework:

| Package | Used By | Purpose |
|---------|---------|---------|
| `chassis` | CLI | Version gate (`RequireMajor(11)`) |
| `chassis-go/v11/errors` | Library | Typed error constructors with HTTP + gRPC codes |
| `chassis-go/v11/secval` | Library | JSON response security validation |
| `chassis-go/v11/call` | CLI | Resilient HTTP client (retry + timeout + OTel tracing) |
| `chassis-go/v11/config` | CLI | Struct-tag-based environment variable config loader |
| `chassis-go/v11/logz` | CLI | Structured JSON logger |
| `chassis-go/v11/testkit` | CLI tests | Test environment helpers |
